...
pub async fn require_admin(user_id: Option<uuid::Uuid>, state: &AppState) -> Option<HttpResponse> {
    ...
    match is_admin_user(user_id, &state).await {
        false => unauthorized("No admin permissions!".to_string()),
        true => None,
    }
}
...
pub async fn require_sensor_permission(
    ...
    let login_id = require_login(user_id, &state)
        .await
...
// TODO: Workaround until consistent error handling is added..
...
pub fn internal_error(error_msg: String) -> HttpResponse {
