...
pub async fn delete_role(
    ...
    let user_res =
        sqlx::query_scalar("DELETE FROM user_roles WHERE role_id = $1 RETURNING user_id")
            .bind(role.id)
            .fetch_all(&mut *tx)
            .await
            .map_err(|err: sqlx::Error| err.to_string());
...
pub async fn assign_role(
    ...
    if role.system && !allow_system {
        anyhow::bail!(
            "System role {} can't be assigned to user {}!",
            role_id,
            user_id
        );
    }
...
pub async fn revoke_role(
    ...
    if role.system && !allow_system {
        anyhow::bail!(
            "System role {} can't be revoked from user {}!",
            role_id,
            user_id
        );
    }
...
pub async fn list_roles(state: &AppState) -> Result<Vec<Role>> {
    let query_res = sqlx::query_as::<_, Role>("SELECT * FROM roles")
        .fetch_all(&state.db)
        .await;
...
pub async fn get_user_roles(user_id: uuid::Uuid, conn: &mut PgConnection) -> Result<Vec<Role>> {
    let query_res = sqlx::query_as::<_, Role>("SELECT roles.id, roles.name, roles.system FROM roles, user_roles WHERE user_roles.user_id=$1 AND user_roles.role_id = roles.id")
        .bind(user_id)
        .fetch_all(conn)
        .await;
