...
use crate::database::models::sensor::{
    ColumnIngest, ColumnType, FullSensorInfo, SensorColumn, ShortSensorInfo,
};
use crate::database::models::user::UserInfo;
use crate::features::cache;
use crate::features::sensor_col_ingest::{
    register_sensor_col_ingest, unregister_sensor_col_ingest,
};
use crate::features::sensor_data_storage::{
    register_sensor_data_storage, unregister_sensor_data_storage,
};
use crate::features::user_sens_perm::{UserSensorPerm, UserSensorPermissions};
use crate::handler::models::requests::{
    CreateApiKeyRequest, CreateSensorRequest, EditSensorRequest, SensorPermissionRequest,
};
...
use sqlx::{query_builder::QueryBuilder, PgConnection, Postgres, Row, Transaction};
use tracing::error;

/// Return a list of all registered sensors.
pub async fn get_sensor_overview(state: &AppState) -> anyhow::Result<Vec<ShortSensorInfo>> {
    //TODO: Can we cache this as well?
    let query_result =
        sqlx::query_as::<_, ShortSensorInfo>("SELECT id, name, longitude, latitude FROM sensor s")
            .fetch_all(&state.db)
            .await
            .unwrap_or_default();
    Ok(query_result)
...
pub async fn get_full_sensor_info(
    sensor_id: uuid::Uuid,
    conn: &mut PgConnection,
) -> anyhow::Result<FullSensorInfo> {
    // Retrieve the data from the sensor and sensor_schema tables
    let query_result = sqlx::query(r#"
        SELECT id, s.name AS name, tbl_name, longitude, latitude, description, owner, storage_type, storage_params, col_name, col_type, col_unit, col_ingest
        FROM sensor s
            JOIN sensor_schema c ON s.id = c.sensor_id
        WHERE s.id = $1"#)
        .bind(sensor_id)
        .fetch_all(&mut *conn)
        .await
    ...
    for row in query_result {
        let c_type: i32 = row.get("col_type");
        let c_ingest: i32 = row.get("col_ingest");

        columns.push(SensorColumn {
            name: row.get("col_name"),
            val_type: ColumnType::from_integer(c_type),
            val_unit: row.get("col_unit"),
            val_ingest: ColumnIngest::from_integer(c_ingest),
        });
    }

    // Get sensor permissions

    let perm_res = sqlx::query_as::<_, SensorPermission>(
        "SELECT * FROM sensor_permissions WHERE sensor_id=$1",
    )
    .bind(sensor_id)
    .fetch_all(&mut *conn)
    .await;
    ...
    if let Err(err) = perm_res {
        error!("Couldn't fetch permissions for sensor {}!", sensor_id);
        anyhow::bail!(err)
    }
...
pub async fn delete_sensor(sensor_id: uuid::Uuid, state: &AppState) -> anyhow::Result<()> {
    let sensor = cache::request_sensor(sensor_id, &state)
        .await
        .ok_or_else(|| anyhow::anyhow!("No sensor with id {}!", sensor_id))?;
    ...
    let query_result = sqlx::query(r#"DELETE FROM sensor_schema WHERE sensor_id = $1"#)
    ...
    // next, remove data column ingest modes

    for col in sensor.columns {
        let res = unregister_sensor_col_ingest(
            col.val_ingest,
            sensor.tbl_name.clone(),
            col.name.clone(),
            tx.as_mut(),
        )
        .await;
    ...
    let res =
        unregister_sensor_data_storage(sensor.storage_type, sensor.tbl_name.clone(), tx.as_mut())
            .await;
    ...
    let query_result = sqlx::query(r#"DELETE FROM sensor_permissions WHERE sensor_id = $1"#)
        .bind(sensor_id.clone())
        .execute(&mut *tx)
        .await
        .map_err(|err: sqlx::Error| err.to_string());
    ...
    let query_result = sqlx::query("DELETE FROM api_keys WHERE sensor_id = $1")
        .bind(sensor_id.clone())
        .execute(&mut *tx)
        .await
        .map_err(|err: sqlx::Error| err.to_string());
    ...
    let query_result = sqlx::query(r#"DELETE FROM sensor WHERE id = $1"#)
        .bind(sensor_id.clone())
        .execute(&mut *tx)
        .await
        .map_err(|err: sqlx::Error| err.to_string());
    ...
    let _ = tx.commit().await; // TODO warum kein ? wird der error hier nicht ignorierT?
...
pub async fn edit_sensor(
    ...
    if sensor.storage_type != body.storage.variant || sensor.storage_params != body.storage.params {
        // Remove old strategy
        let res = unregister_sensor_data_storage(
            sensor.storage_type,
            sensor.tbl_name.clone(),
            tx.as_mut(),
        )
        ...
        if let Err(err) = res {
            let _ = tx.rollback().await;
            error!(
                "Couldn't remove old data storage strategy for sensor with id {}!",
                sensor_id
            );
            anyhow::bail!(err);
        }
        ...
        // Add new strategy
        let res = register_sensor_data_storage(
        ...
        if let Err(err) = res {
            let _ = tx.rollback().await;
            error!(
                "Couldn't setup new data storage for sensor with id {}!",
                sensor_id
            );
            anyhow::bail!(err);
        }
    ...
    if let Err(err) = perm_query {
        let _ = tx.rollback().await;
        error!("Couldn't set permissions for sensor with id {}!", sensor_id);
        anyhow::bail!(err);
    }
...
r#"INSERT INTO sensor (id, name, description, longitude, latitude, tbl_name, owner, storage_type, storage_params)
    ...
    if let Err(err) = query_result {
        let _ = tx.rollback().await;
        error!("INSERT INTO sensor failed");
        anyhow::bail!(err)
    }
    ...
    // Define created_at timestamp and reject all manually provided future timestamps
    create_stmt.push_str(format!("CREATE TABLE {} (created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, CHECK(created_at <= CURRENT_TIMESTAMP)", table_name).as_str());

    let columns = &body.columns;
    for col in columns.iter() {
        create_stmt.push_str(", ");
        create_stmt.push_str(format!("{} {}", &col.name, col.val_type.to_sql_type()).as_str());

        let query_result = sqlx::query(
            r#"INSERT INTO sensor_schema (sensor_id, col_name, col_type, col_unit, col_ingest)
            VALUES ($1, $2, $3, $4, $5)
            "#,
        )
        .bind(sensor_id.clone())
        .bind(col.name.to_string())
        .bind(col.val_type.to_sql_value())
        .bind(col.val_unit.to_string())
        .bind(col.val_ingest.to_sql_value())
        .execute(&mut *tx)
        .await
        .map_err(|err: sqlx::Error| err.to_string());
        ...
        if let Err(err) = query_result {
            let _ = tx.rollback().await;
            error!("INSERT INTO sensor_schema failed");
            anyhow::bail!(err)
        }
    }
    ...
    if let Err(err) = query_result {
        let _ = tx.rollback().await;
        error!("CREATE TABLE sensor... failed");
        anyhow::bail!(err)
    }

    // Register data column ingest modes

    for col in columns.iter() {
        let col_ingest_res = register_sensor_col_ingest(
            col.val_ingest,
            table_name.clone(),
            col.name.clone(),
            tx.as_mut(),
        ...
        if let Err(err) = col_ingest_res {
            let _ = tx.rollback().await;
            error!(
                "Couldn't setup data ingest mode {:?} for col {} of sensor {}!",
                col.val_ingest, col.name, sensor_id
            );
            anyhow::bail!(err);
        }
    }
    ...
    let data_storage_res = register_sensor_data_storage(
        body.storage.variant,
        table_name,
        body.storage.params,
        tx.as_mut(),
    ...
    if let Err(err) = data_storage_res {
        let _ = tx.rollback().await;
        error!(
            "Couldn't setup data storage for sensor with id {}!",
            sensor_id
        );
        anyhow::bail!(err);
    }
...
pub async fn get_user_sensor_permissions(
    user_id: Option<uuid::Uuid>,
    sensor: &FullSensorInfo,
    state: &AppState,
) -> UserSensorPermissions {
    let user = match user_id {
        Some(u) => {
            let user = cache::request_user(u, &state).await;

            if user.is_none() {
                error!("User not found with id {}!", user_id.unwrap());
...
/// Retrieves a list of permissions the user has for the specified sensor.
pub async fn get_user_sensor_permissions_impl(
    ...
    let guest_role = cache::request_role(ROLE_SYSTEM_GUEST, &state)
        .await
        .unwrap();
        ...
        for perm in sensor.permissions.iter() {
            if perm.role_id == role.id {
                ...
                if perm.allow_read {
                    ...
                    if perm.role_id != guest_role.id {
                        // Guests can't create keys
...
pub async fn update_sensor_access(
    sensor_id: uuid::Uuid,
    ...
    for permission in permissions {
        let role = cache::request_role(permission.role_id, &state)
            .await
            .expect(&format!(
                "Couldn't find role with name {}!",
                permission.role_id
            ));
        ...
        if ex_perm.is_some() {
            ...
            if ex_perm.allow_info == allow_info
                && ex_perm.allow_read == allow_read
                && ex_perm.allow_write == allow_write
            {
                continue;
            }
        }
        ...
        if !allow_info && !allow_read && !allow_write {
            ...
            let query_res =
                sqlx::query("DELETE FROM sensor_permissions WHERE sensor_id=$1 AND role_id=$2")
                    .bind(sensor_id)
                    .bind(role.id)
                    .execute(&mut **tx)
                    .await
            ...
            if let Err(err) = query_res {
                error!(
                    "Couldn't remove permissions for sensor id {} and role id {}!",
                    sensor_id, role.id
                );
                anyhow::bail!(err)
            }
            ...
            let query_res = sqlx::query(
                r#"INSERT INTO sensor_permissions(sensor_id, role_id, allow_info, allow_read, allow_write)
            VALUES($1, $2, $3, $4, $5)
            ...
            if let Err(err) = query_res {
                error!(
                    "Couldn't update permissions for sensor id {} and role id {}!",
                    sensor_id, role.id
                );
                anyhow::bail!(err)
            }
...
pub async fn get_api_keys(
    ...
    let query_res =
        sqlx::query_as::<_, ApiKey>("SELECT * FROM api_keys WHERE sensor_id=$1 AND user_id=$2")
            .bind(sensor_id)
            .bind(user_id)
            .fetch_all(&state.db)
            .await;
    ...
    match query_res {
        ...
        Err(err) => {
            error!(
                "Couldn't fetch api_keys for sensor {} and user {}!",
                sensor_id, user_id
            );
            anyhow::bail!(err);
        }
...
pub async fn get_api_key(key_id: uuid::Uuid, state: &AppState) -> anyhow::Result<ApiKey> {
    let query_res = sqlx::query_as::<_, ApiKey>("SELECT * FROM api_keys WHERE id=$1")
        .bind(key_id)
        .fetch_one(&state.db)
        .await;
    ...
    match query_res {
        Ok(key) => Ok(key),
        Err(err) => {
            error!("Couldn't fetch api_key with id {}!", key_id);
            anyhow::bail!(err);
        }
    }
}
...
pub async fn create_api_key(
    ...
    let query_result = sqlx::query("INSERT INTO api_keys VALUES($1, $2, $3, $4, $5)")
        .bind(&new_key.id)
    ...
    if let Err(err) = query_result {
        error!(
            "Couldn't create api key for sensor id {} and user id {}!",
            sensor_id, user_id
        );
        anyhow::bail!(err);
    }
...
pub async fn delete_api_keys(
    ...
    let mut query_builder: QueryBuilder<Postgres> =
        QueryBuilder::new("DELETE FROM api_keys WHERE id IN ( ");
    ...
    if let Err(err) = query_result {
        error!("Couldn't delete api keys {:?}!", keys);
        anyhow::bail!(err)
    }
...
pub async fn validate_api_keys(
    ...
    let query = match (&sensor, &user) {
        ...
        (Some(sens), None) => {
            sqlx::query_as::<_, ApiKey>("SELECT * FROM api_keys WHERE sensor_id=$1")
                .bind(sens.id)
                .fetch_all(&mut *ex)
                .await
        }
        (None, Some(us)) => {
            sqlx::query_as::<_, ApiKey>("SELECT * FROM api_keys WHERE user_id=$1")
                .bind(us.id)
                .fetch_all(&mut *ex)
                .await
        }
    ...
    if let Err(err) = query_res {
        error!(
            "Couldn't fetch api keys for sensor {:?} and user {:?}!",
            sensor, user
        );
        anyhow::bail!(err)
    }
    ...
    for key in query_res.unwrap() {
        ...
        if s.is_none() {
            anyhow::bail!(
                "Couldn't get sensor {} for api_key {}!",
                key.sensor_id,
                key.id
            )
        }
        ...
        let permissions =
            get_user_sensor_permissions_impl(u.as_ref(), s.as_ref().unwrap(), &state).await;
    ...
    if !keys_to_delete.is_empty() {
        ...
        if res.is_err() {
            error!("Couldn't delete API keys {:?}!", keys_to_delete);
