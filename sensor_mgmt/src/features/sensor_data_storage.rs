...
pub async fn register_sensor_data_storage(
    storage_type: SensorDataStorageType,
    table_name: String,
    ...
    match storage_type {
        ...
        SensorDataStorageType::RingBufferCount => {
            RING_BUFFER_COUNT_OBJ
                .on_initialize(table_name, params, ex)
                .await
        }
        SensorDataStorageType::RingBufferInterval => {
            RING_BUFFER_INTERVAL_OBJ
                .on_initialize(table_name, params, ex)
                .await
        }
    }
...
pub async fn unregister_sensor_data_storage(
    storage_type: SensorDataStorageType,
    table_name: String,
    ex: &mut PgConnection,
) -> Result<()> {
    match storage_type {
        ...
        SensorDataStorageType::RingBufferCount => {
            RING_BUFFER_COUNT_OBJ.on_remove(table_name, ex).await
        }
        SensorDataStorageType::RingBufferInterval => {
            RING_BUFFER_INTERVAL_OBJ.on_remove(table_name, ex).await
        }
    }
}
...
impl SensorDataStorageStrategy for RingBufferCountSDS {
    async fn on_initialize(
        ...
        let res = sqlx::query(
            format!(
                "select create_ring_buffer_count('{}', {});",
                table_name, count
            )
            .as_str(),
        )
    ...
    async fn on_remove(&self, table_name: String, ex: &mut PgConnection) -> Result<()> {
        let res = sqlx::query(
            format!("drop function check_data_storage_{}() cascade;", table_name).as_str(),
        )
        .execute(&mut *ex)
        .await
        .map_err(|err: sqlx::Error| err.to_string());
...
impl SensorDataStorageStrategy for RingBufferIntervalSDS {
    async fn on_initialize(
        ...
        let res = sqlx::query(
            format!(
                "select create_ring_buffer_interval('{}', {});",
                table_name, range
            )
            .as_str(),
        )
...
#[cfg(test)]
pub mod tests {
    use crate::database::{data_db, sensor_db};
    ...
    use crate::test_utils::tests::{add_dummy_data, create_test_app, create_test_sensors};
    use serde_json::json;
    use sqlx::PgPool;
    use tokio::time;
    ...
    async fn test_default(pool: PgPool) {
        ...
        // Invalid param
        assert!(sensor_db::edit_sensor(
            sensor_id,
            edit_sensor_req(SensorDataStorageCfg {
                variant: SensorDataStorageType::Default,
                params: json!({"ff": 10}).as_object().cloned()
            }),
            &state
        )
        .await
    ...
    async fn test_ring_buffer_count(pool: PgPool) {
        ...
        assert!(sensor_db::edit_sensor(
            ...
            edit_sensor_req(SensorDataStorageCfg {
                ...
                params: json!({"asd": 10}).as_object().cloned()
            }),
            &state
        )
        .await
        ...
        assert!(sensor_db::edit_sensor(
            ...
            edit_sensor_req(SensorDataStorageCfg {
                variant: SensorDataStorageType::RingBufferCount,
                params: json!({"count": "123"}).as_object().cloned()
            }),
            &state
        )
        .await
        ...
        assert!(sensor_db::edit_sensor(
            ...
            edit_sensor_req(SensorDataStorageCfg {
                variant: SensorDataStorageType::RingBufferCount,
                params: json!({"count": 1234.1234}).as_object().cloned()
            }),
            &state
        )
        .await
        ...
        assert!(sensor_db::edit_sensor(
            ...
            edit_sensor_req(SensorDataStorageCfg {
                variant: SensorDataStorageType::RingBufferCount,
                params: json!({"count": 10, "prop": "value"}).as_object().cloned()
            }),
            &state
        )
        .await
        ...
        assert_eq!(
            sensor.storage_params,
            json!({"count": 10}).as_object().cloned()
        );
    ...
    async fn test_ring_buffer_range(pool: PgPool) {
        ...
        assert!(sensor_db::edit_sensor(
            ...
            edit_sensor_req(SensorDataStorageCfg {
                ...
                params: json!({"asdc": 10}).as_object().cloned()
            }),
            &state
        )
        .await
        ...
        assert!(sensor_db::edit_sensor(
            ...
            edit_sensor_req(SensorDataStorageCfg {
                variant: SensorDataStorageType::RingBufferInterval,
                params: json!({"interval": "123"}).as_object().cloned()
            }),
            &state
        )
        .await
        ...
        assert!(sensor_db::edit_sensor(
            ...
            edit_sensor_req(SensorDataStorageCfg {
                variant: SensorDataStorageType::RingBufferInterval,
                params: json!({"interval": 123.123}).as_object().cloned()
            }),
            &state
        )
        .await
        ...
        assert!(sensor_db::edit_sensor(
            ...
            edit_sensor_req(SensorDataStorageCfg {
                variant: SensorDataStorageType::RingBufferInterval,
                params: json!({"interval": 10, "prop": "value"})
                    .as_object()
                    .cloned()
            }),
            &state
        ...
        assert_eq!(
            sensor.storage_params,
            json!({"interval": 600.0 / 60_000.0}).as_object().cloned()
        );

        add_dummy_data(6, 0, 10, sensor_id, &state).await; // Tuples that will be evicted
        time::sleep(core::time::Duration::from_millis(600)).await; // Clear delay to avoid fluctuation in runtimes
        ...
        time::sleep(core::time::Duration::from_millis(1_200)).await; // Clear delay to avoid fluctuation in runtimes
