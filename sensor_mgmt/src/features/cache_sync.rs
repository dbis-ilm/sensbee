...
impl CacheSyncData {
    pub fn new(cache: Arc<CachedData>, pool: PgPool) -> CacheSyncData {
        ...
        let reg: Arc<RwLock<HashMap<MapType, EventHandlerCallback>>> =
            Arc::new(RwLock::new(HashMap::default()));

        let tasks: Arc<Mutex<HashMap<String, Arc<(Mutex<bool>, Condvar)>>>> =
        ...
        MapType::Roles => {
            if se.data.len() == 0 {
                ...
                cache
                    ...
                    .write()
                    .unwrap()
                    .remove(&Uuid::parse_str(&se.data).unwrap());
            }
        }
            ...
            if se.data.len() == 0 {
                ...
                if d.is_sensor_id {
                    ...
                    cache
                        ...
                        .write()
                        .unwrap()
                        .retain(|_, s| s.owner.is_none() || s.owner.unwrap() != d.key);
                }
            }
        }
            ...
            if se.data.len() == 0 {
                ...
                if d.is_user {
                    ...
                    cache
                        ...
                        .write()
                        .unwrap()
                        .retain(|_, v| v.user_id != d.key);
                }
            }
        }
        _ => panic!(
            "synced_apikeys_purge received unexpected target type {}",
            se
        ),
    }
}
